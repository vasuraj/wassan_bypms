...
class ImagesController extends Controller {
	...
	$path = public_path('uploads/cropped') . '/' . $filename;
	...
	$path = public_path('uploads/thumb') . '/' . $filename;
	...
	public function image_save_to_file(Request $request)
	{
		...
		if (Request::ajax())
		{
			...
			if ( in_array($extension, $allowedExts))
			  ...
			  if ($_FILES["img"]["error"] > 0)
				...
				$filename ='img_'.date('d_m_Y_h_i_s_A').'.'.$extension;
...
//$output_filename = public_path('uploads')."/croppedImg_".rand().date('d_m_Y_h_i_s_A');
//echo $output_filename;
$output_filename = public_path('uploads')."/cropped/".str_replace('.'.$extension,'',$filename);



$what = getimagesize($imgUrl);

//$what['mime']=$mime = Image::make(public_path('uploads').'/'.'Lighthouse.jpg')->mime();
...
if(!is_writable(dirname($output_filename))){
	...
	$outputFilename=str_replace(public_path('uploads').'/','',$output_filename.$type);
	$outputUrl=URL::to('image/get_image').'/'.$outputFilename;

	// create instance
	$img = Image::make(public_path('uploads').'/'.$outputFilename);
	...
	$img->save(public_path('uploads').'/thumb'.str_replace('cropped/', '', $outputFilename));
