	...
	public function seed_purchaser_form()
	{
		...
		elseif($_POST['select_option']==='by_adhar_number')
		{
			if($_POST['adhar_number']!='' && $_POST['adhar_number']!='none')
			{
				...
				$data['farmer']=DB::table('farmers')->where('adhar_card_number','=',$adhar_number)->first();
...
echo "</script>";
	...
	public function store_rouging($transaction_id=null)
	{
			...
			$data['sowing_date']=date_create_from_format('d/m/Y',$data['sowing_date'])->format('Y-m-d');
...
public function farmers_list()
	{
		...
		foreach($farmers_list as $farmer)
		{
			 ...
			 $farmers_list_html.="<option data-content=\"<img style='width:50px;' src='".$farmer->farmer_image."'> $farmer->name s/o $farmer->fname \" value='$farmer->id'>$farmer->name/$farmer->fname</option>";
