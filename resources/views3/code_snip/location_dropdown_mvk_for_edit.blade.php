...
<?php
 ...
 $mvks=get_mvks($selected_mandal_id);

?>
<div id="mvk_area" class="col-md-3">
<label class="control-label" for="mandal">MVK</label>

<select class="form-control form-group" name="mvk" id="mvk">

 <option value='' data-id='none'  <?php if(isset($user)){ if($user->mvk==''){$selected_mvk_id=''; }} ?>     @if(isset($user))  @if($user->mvk==='')   selected @endif  @endif >
	Do Not Mention
 </option>
 ...
 <option value='{{$mvk->name}}'  <?php if(isset($user)){ if($mvk->name===$user->mvk){$selected_mvk_id=$mvk->id;}}?>   data-id="{{$mvk->id}}" @if(isset($user)) @if($user->mvk===$mvk->name)   selected @endif  @endif >
